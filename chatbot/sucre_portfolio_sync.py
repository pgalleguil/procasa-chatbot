...
This module is independent from ``ficha_sync_loop``.  Embeddings are loaded
only on an explicit non-dry-run operational execution, for active properties
that are new, have a real published-price change, or are missing a vector.
They are never generated by a scheduler or by a dry-run.
"""
...
EMBEDDING_FIELD = "vector_descripcion"
EMBEDDING_PENDING_FIELD = "embedding_pending"
...
def _listing_snapshot(row: dict) -> dict:
    """Capture the fields used by the operational listing comparator."""
    return {
...
def _has_embedding(doc: dict) -> bool:
    """Return whether a usable semantic-search vector is already stored."""
    vector = doc.get(EMBEDDING_FIELD)
    return isinstance(vector, (list, tuple)) and len(vector) > 0


def _generate_embedding_for_doc(doc: dict) -> tuple[list[float] | None, str | None]:
    """Generate one vector lazily for an explicit operational write.

    The import stays inside this function so classification and dry-runs do
    not load FastEmbed/ONNX.  A failed embedding is reported to the caller so
    the property update can remain retryable through ``embedding_pending``.
    """
    try:
        from chatbot.semantic_engine import build_embedding_text, generate_embedding
    except Exception as exc:  # pragma: no cover - depends on deployment image
        log.exception("No se pudo importar el motor de embeddings")
        return None, f"import_{type(exc).__name__}"

    try:
        text = build_embedding_text(doc)
        if not text:
            return None, "empty_embedding_text"
        vector = generate_embedding(text)
        if not vector:
            return None, "model_unavailable_or_generation_failed"
        return list(vector), None
    except Exception as exc:  # pragma: no cover - model/runtime dependent
        log.exception("Fallo generando embedding operativo")
        return None, type(exc).__name__


def _register_embedding_result(
    result: dict,
    code: str,
    ...
    if vector is not None:
        result["embeddings_generados"] += 1
        return
    result["embeddings_fallidos"] += 1
    result["embedding_error_codes"].append(
        {"codigo": str(code), "error": error or "unknown_embedding_error"}
    )
...
def _empty_result(started_at: datetime, office_ids: tuple[int, ...]) -> dict:
    return {
        ...
        "embeddings_requeridos": 0,
        "embeddings_generados": 0,
        "embeddings_fallidos": 0,
        "embeddings_omitidos_dry_run": 0,
        "active_embeddings_missing_before": 0,
...
def _price_pair(value: Any) -> dict[str, Any]:
    """Extract the two amounts shown by the listing price cell.

    Prop360 may show the published amount together with its converted value.
    Keeping both values lets the operational comparator recognize a stable
    published amount even when the other currency changes with the UF rate.
    """
    uf, clp = parse_listing_price(value)
    return {"UF": uf, "CLP": clp}


def _snapshot_price_pair(doc: dict) -> dict[str, Any] | None:
    snapshot = _stored_listing_snapshot(doc)
    if not isinstance(snapshot, dict):
        return None
    raw_price = snapshot.get("precio")
    if not _norm_text(raw_price):
        return None
    pair = _price_pair(raw_price)
    return pair if any(value is not None for value in pair.values()) else None


def _stored_price_matches_snapshot(stored: dict | None, snapshot_pair: dict | None) -> bool:
    """Return whether the stored publication value agrees with the old listing.

    This is a confidence check, not a price comparison.  Legacy documents can
    contain a derived amount in ``resumen``; those must not be trusted to
    decide whether the current listing changed.
    """
    if not stored or not snapshot_pair:
        return False
    currency = str(stored.get("moneda") or "").upper()
    if currency not in {"UF", "CLP"}:
        return False
    old_amount = snapshot_pair.get(currency)
    return old_amount is not None and stored.get("monto") is not None and _same_value(
        stored.get("monto"), old_amount
    )


def _publication_currency_is_authoritative(doc: dict, stored: dict | None) -> bool:
    """Whether the stored publication currency can disambiguate a dual quote.

    A printable ficha exposes the published amount and its converted amount,
    but some rows use the same ``$`` glyph for both values.  Its stored
    currency therefore cannot be used to turn a stable-UF/changed-CLP pair
    into a commercial price change.  Editable fichas, on the other hand,
    carry the selected currency in the source form and remain authoritative.
    """
    if not stored or str(stored.get("moneda") or "").upper() not in {"UF", "CLP"}:
        return False
    metadata = doc.get("metadata") or {}
    origin = str(metadata.get("origen_ficha") or "").lower()
    if origin == "ficha_imprimible":
        recovered = _legacy_snapshot_publication_price(doc)
        return bool(
            recovered
            and recovered.get("moneda") == stored.get("moneda")
            and _same_value(recovered.get("monto"), stored.get("monto"))
        )
    return True


def _price_change_payload(
    old_currency: str,
    old_amount: Any,
    new_currency: str,
    new_amount: Any,
    ...
    return {
        ...
        "moneda_anterior": old_currency,
        "monto_anterior": old_amount,
        "moneda_nueva": new_currency,
        "monto_nuevo": new_amount,
        "anterior": old_amount,
        "nuevo": new_amount,
    }
...
def _price_assessment(doc: dict, row: dict) -> dict:
    ...
    # Prefer a pair-to-pair comparison when both the previous listing snapshot
    # and the current listing expose UF and CLP.  A stable UF with a changing
    # CLP (or vice versa) is a conversion update, not a commercial change.
    # Only use the stored canonical currency to override that conservative
    # rule when its amount is demonstrably aligned with the old snapshot; this
    # preserves genuine CLP/UF changes below 1% while rejecting stale legacy
    # amounts that were derived from the other currency.
    snapshot_pair = _snapshot_price_pair(doc)
    current_pair = _price_pair(raw_price)
    if stored:
        stored_currency = str(stored.get("moneda") or "").upper()
        if stored_currency in {"UF", "CLP"} and _same_value(
            stored.get("monto"), current_pair.get(stored_currency)
        ):
            # A previous operational write may have refreshed the canonical
            # amount before older documents' listing snapshot was refreshed.
            # The current published amount is already in Mongo, so do not
            # replay the same event from that stale snapshot.
            return {"change": None, "ambiguous": False}
    if snapshot_pair and all(
        snapshot_pair.get(currency) is not None and current_pair.get(currency) is not None
        for currency in ("UF", "CLP")
    ):
        stable_currencies = [
            currency
            for currency in ("UF", "CLP")
            if _same_value(snapshot_pair[currency], current_pair[currency])
        ]
        if len(stable_currencies) == 2:
            return {"change": None, "ambiguous": False}

        stored_is_trusted = _stored_price_matches_snapshot(stored, snapshot_pair)
        if len(stable_currencies) == 1:
            stable_currency = stable_currencies[0]
            if not stored_is_trusted:
                return {"change": None, "ambiguous": False}
            stored_currency = str(stored.get("moneda") or "").upper()
            if stored_currency == stable_currency:
                return {"change": None, "ambiguous": False}
            if not _publication_currency_is_authoritative(doc, stored):
                # The printable ficha can show a converted CLP amount before
                # a stable UF amount without identifying which one is the
                # published currency.  Do not turn that daily conversion into
                # a commercial change; leave it for explicit review instead.
                return {"change": None, "ambiguous": True}
            # The stored canonical value is aligned with the old snapshot and
            # points to the changing side, so this is a real published-price
            # change (including changes smaller than 1%).
            return {
                "change": _price_change_payload(
                    stored_currency,
                    snapshot_pair[stored_currency],
                    stored_currency,
                    current_pair[stored_currency],
                ),
                "ambiguous": False,
            }

        # Both displayed amounts changed.  Prefer a stable currency marker in
        # the raw listing pair; fall back to a trusted canonical currency.  If
        # neither source can identify the publication currency, do not invent
        # one and leave the row safely ambiguous.
        old_primary = _parse_primary_listing_price(
            ((doc.get("resumen") or {}).get("snapshot_listado") or {}).get("precio")
        )
        new_primary = _parse_primary_listing_price(raw_price)
        currency = None
        if (
            old_primary
            and new_primary
            and old_primary.get("moneda") == new_primary.get("moneda")
        ):
            currency = old_primary["moneda"]
        elif stored_is_trusted:
            currency = str(stored.get("moneda") or "").upper()
        if currency in {"UF", "CLP"}:
            return {
                "change": _price_change_payload(
                    currency,
                    snapshot_pair[currency],
                    currency,
                    current_pair[currency],
                ),
                "ambiguous": False,
            }
        return {"change": None, "ambiguous": True}

    # Fallback for old snapshots that contain only one explicit currency, or
    # for documents without a snapshot.  Here the canonical currency is the
    # only reliable context available.
    if _is_legacy_printable_doc(doc):
    ...
    if stored is not None and stored.get("moneda") == listed.get("moneda"):
        ...
        return {
            "change": _price_change_payload(
                None, None, listed["moneda"], listed["monto"]
            ),
            "ambiguous": False,
        }
    return {
        "change": _price_change_payload(
            stored.get("moneda") if stored else None,
            stored.get("monto") if stored else None,
            listed["moneda"],
            listed["monto"],
        ),
        "ambiguous": False,
...
def _candidate_after_set_ops(existing: dict, set_ops: dict) -> dict:
    """Build the document version that will be represented by a new vector."""
...
def _embedding_update_ops(doc: dict, code: str, result: dict) -> dict:
    """Return the Mongo fields for one embedding attempt and track its result."""
    vector, error = _generate_embedding_for_doc(doc)
    _register_embedding_result(result, code, vector, error)
    if vector is not None:
        return {
            EMBEDDING_FIELD: vector,
            EMBEDDING_PENDING_FIELD: False,
        }
    # Keep the operational change, but make the missing vector explicit so a
    # later manual run can retry it without requiring another price change.
    return {EMBEDDING_PENDING_FIELD: True}
...
def _operational_update(
    ...
    set_ops["resumen.snapshot_listado"] = snapshot
    set_ops["resumen.listing_fingerprint"] = listing_fingerprint(snapshot)
...
def run_portfolio_operational_sync(
    *,
    ...
    if generate_embeddings is None:
        generate_embeddings = not dry_run
    embedding_allowed = bool(generate_embeddings and not dry_run)
    result["embeddings_habilitados"] = embedding_allowed
    ...
    try:
        ...
        embedding_updates_done: set[tuple[int, str]] = set()
        for scope in scopes:
        ...
        # A real run keeps semantic search complete for the active portfolio:
        # new properties and real price changes are mandatory targets, while
        # already-active documents without a vector are backfilled one by one
        # from their stored full document.  This is still separate from the
        # historical/offline bulk embedding job and never runs in dry-run.
        embedding_targets: dict[tuple[int, str], str] = {}
        active_missing = 0
        for scope in scopes:
            for code in scope["new_codes"]:
                embedding_targets[(scope["office_id"], str(code))] = "new"
            for update in scope["updates"]:
                if update.get("price_change"):
                    embedding_targets.setdefault(
                        (scope["office_id"], str(update["codigo"])), "price_change"
                    )
            for code in scope["active"]:
                existing_doc = scope["existing"].get(code)
                if existing_doc is not None and not _has_embedding(existing_doc):
                    active_missing += 1
                    embedding_targets.setdefault(
                        (scope["office_id"], str(code)), "active_missing"
                    )
        result["active_embeddings_missing_before"] = active_missing
        result["embeddings_requeridos"] = len(embedding_targets)
        if not embedding_allowed:
            result["embeddings_omitidos_dry_run"] = len(embedding_targets) if dry_run else 0
        ...
        for scope in scopes:
            for code in scope["new_codes"]:
                ...
                try:
                    ...
                    if embedding_allowed:
                        doc.update(_embedding_update_ops(doc, code, result))
                    if dry_run:
            ...
            for update in scope["updates"]:
                ...
                if not set_ops:
                    continue
                if not dry_run:
                    embedding_key = (scope["office_id"], str(code))
                    if embedding_allowed and embedding_key in embedding_targets:
                        candidate = _candidate_after_set_ops(
                            scope["existing"][code], set_ops
                        )
                        set_ops.update(
                            _embedding_update_ops(candidate, code, result)
                        )
                        embedding_updates_done.add(embedding_key)
            ...
            # Complete vectors for active documents that had no operational
            # change.  This writes only the vector metadata and creates no
            # commercial/history event.
            if embedding_allowed:
                for code in scope["active"]:
                    embedding_key = (scope["office_id"], str(code))
                    if embedding_key not in embedding_targets or embedding_key in embedding_updates_done:
                        continue
                    existing_doc = scope["existing"].get(code)
                    if existing_doc is None:
                        continue
                    embedding_ops = _embedding_update_ops(existing_doc, code, result)
                    _write_events_and_update(
                        coll,
                        history_coll,
                        [],
