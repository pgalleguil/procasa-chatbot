...
from chatbot.captacion_reminder import (DOMAIN, MESSAGE_TYPE, RECIPIENT_ROLE,
                                        has_degraded_unicode, property_summary, reminder_text)
...
def test_property_summary_formats_uf_without_unnecessary_decimals():
    ...
    assert property_summary({"codigo":"6132","tipo_propiedad":"casa","comuna":"Santiago",
                             "operacion":"venta","precio_uf":3393.0}) == separator.join(["6132","Casa","Santiago","3.393 UF"])
    assert property_summary({"codigo":"X","operacion":"arriendo","precio_uf":1713.8}) == separator.join(["X","1.713,8 UF"])
...
def test_reminder_message_keeps_utf8_state_note_and_absolute_url():
    ...
    cap = {"codigo":"6132","tipo_propiedad":"casa","comuna":"Santiago","operacion":"venta","precio_uf":3393.0,"gestion":{"estado_captacion":"Sin respuesta"}}
    ...
    assert "6132 " + chr(0x00b7) + " Casa " + chr(0x00b7) + " Santiago " + chr(0x00b7) + " 3.393 UF" in text
