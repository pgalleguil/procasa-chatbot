...
class AppConfig:
    ...
    classification_cache_enabled: bool = field(default_factory=lambda: _env_bool("CLASSIFICATION_CACHE_ENABLED", True))
    classification_cache_path: str = field(default_factory=lambda: os.getenv("CLASSIFICATION_CACHE_PATH", ""))
    extractor_health_min_sample: int = field(default_factory=lambda: _env_int("EXTRACTOR_HEALTH_MIN_SAMPLE", 20))
    extractor_health_drop_ratio: float = field(default_factory=lambda: _env_float("EXTRACTOR_HEALTH_DROP_RATIO", 0.35))
    extractor_health_baseline_path: str = field(default_factory=lambda: os.getenv("EXTRACTOR_HEALTH_BASELINE_PATH", ""))
    max_ai_calls_per_run: int = field(default_factory=lambda: _env_int("MAX_AI_CALLS_PER_RUN", 50))
    max_input_tokens_per_run: int = field(default_factory=lambda: _env_int("MAX_INPUT_TOKENS_PER_RUN", 100000))
    max_output_tokens_per_run: int = field(default_factory=lambda: _env_int("MAX_OUTPUT_TOKENS_PER_RUN", 20000))
    max_estimated_cost_per_run: float = field(default_factory=lambda: _env_float("MAX_ESTIMATED_COST_PER_RUN", 5.0))
    estimated_input_cost_per_million: float = field(default_factory=lambda: _env_float("ESTIMATED_INPUT_COST_PER_MILLION", 0.14))
    estimated_output_cost_per_million: float = field(default_factory=lambda: _env_float("ESTIMATED_OUTPUT_COST_PER_MILLION", 0.28))
