        ...
        for tp in tipos:
            ...
            for page in range(1, args.max_pages + 1):
                ...
                for prop in props:
                    ...
                    # All classification, cache, identity and AI decisions go
                    # through the central service. This legacy discovery route
                    # does not have a complete-batch health preflight yet, so
                    # residual AI is intentionally held pending rather than
                    # bypassing the extractor gate.
                    rule_result = (
                        classify_structural_broker(extracted)
                        or classify_structural_owner(extracted)
                        or classify_obvious_broker(extracted)
                    )
                    ...
                    if central_result.get("ai_called"):
                        deepseek_calls += 1
                    if central_result.get("reason") == "deepseek_error":
                        deepseek_errors += 1
                    if central_result.get("reason") in {"EXTRACTOR_DEGRADED", "EXTRACTOR_HEALTH_UNAVAILABLE"}:
                        fallbacks_used += 1
