...
async def create_contract(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        try:
            ...
            # Subir a Drive: carpeta plana directamente bajo la raíz [prop_code]_[cliente]
            client_name = data.get("cliente_nombre", "")
            prop_code = property_code or data.get("propiedad_codigo", "")
            folder_name = f"{sanitize_folder_name(prop_code, 'Prop')}_{sanitize_folder_name(client_name, 'Cliente')}"
            folder_id = await run_in_threadpool(gdrive_sync.create_subfolder, gdrive_sync.parent_folder_id, folder_name)
            ...
            if folder_id and folder_id != "mock_folder_id":
                ...
                if file_id and file_id != "mock_file_id":
                    ...
                    logger.info(f"[VISITAS] PDF subido a Drive code={visita_code} folder={folder_name} file_id={file_id}")
                else:
                    logger.error(f"[VISITAS] upload_file devolvio ID invalido para {visita_code}")
            else:
                logger.error(f"[VISITAS] No se pudo crear carpeta Drive para {visita_code}")
            ...
            import traceback
            logger.error(f"[VISITAS] Error generando original: {e_pdf}\n{traceback.format_exc()}")
        ...
        try:
            ...
            if folder_id:
                ...
                if new_file_id and new_file_id != "mock_file_id":
                    ...
                    logger.info(f"[DOWNLOAD] PDF regenerado y subido a carpeta de expediente code={visita_code} file_id={new_file_id}")
        except Exception as e:
...
def active_token_query(token: str) -> dict:
    """Find only non-deleted visit orders addressed by a signing token."""
    return {
        "security.token": token,
        "status": {"$ne": "deleted"},
    }
...
async def view_visita_public(token: str, request: Request):
    ...
    db = get_db()
    contract = await _db_call(db["visitas"].find_one, active_token_query(token))
...
async def request_otp(token: str, request: Request, background_tasks: BackgroundTasks):
    ...
    from chatbot.storage import get_async_db
    adb = get_async_db()
    contract = await adb["visitas"].find_one(active_token_query(token))
    if not contract:
...
async def accept_contract(token: str, request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if perm_path_from_db and os.path.exists(perm_path_from_db):
            ...
            original_bytes = await run_in_threadpool(PDFGenerator.generate_original_contract, data_payload)
        ...
        signed_pdf_bytes = await run_in_threadpool(PDFGenerator.generate_signed_contract, original_bytes, contract, evidence_data, verify_url)
...
def _get_or_create_expedition_folder(db, collection, code_field, code, client_name, property_code):
    ...
    Carpeta Raíz (Ordenes de Visita) -> [Código Propiedad] -> [Nombre Cliente]
    """
    try:
...
@router.delete("/api/{visita_code}/delete")
