...
except ImportError:  # pragma: no cover - direct local-script compatibility
    from discovery import _extract_next_data, extract_metadata_from_next_data
...
def _extract_from_next_data(next_data: dict[str, Any], source_url: str) -> dict[str, Any]:
    ...
    # The current SEO route renders a detail page whose authoritative data is
    # under initialState.property.property.data, not pageProps.propiedades.
    # Preserve the numeric portal id and structured address from that payload;
    # the visible SEO URL itself only contains a non-canonical hash id.
    try:
        detail = (
            next_data.get("props", {})
            .get("pageProps", {})
        ...
        if isinstance(detail, dict) and detail:
            ...
            if detail.get("title") and not fields.get("title"):
                fields["title"] = str(detail["title"])
            if detail.get("description") and not fields.get("description"):
                fields["description"] = str(detail["description"])
            if detail.get("priceUf") not in (None, ""):
                fields["price_uf"] = f"UF {detail['priceUf']}"
            if detail.get("price") not in (None, ""):
                fields["price_clp"] = f"$ {detail['price']}"
            address = detail.get("address") if isinstance(detail.get("address"), dict) else {}
            if address.get("commune"):
                fields["comuna"] = str(address["commune"])
                fields["comuna_source"] = "detail_next_data"
                if address.get("communeId") not in (None, ""):
                    fields["comuna_id"] = str(address["communeId"])
            if address.get("region"):
                fields["region"] = str(address["region"])
                fields["region_source"] = "detail_next_data"
            client = detail.get("client") if isinstance(detail.get("client"), dict) else {}
            if client.get("name") and not fields.get("publicador_visible"):
                fields["publicador_visible"] = str(client["name"])
...
def _extract_comuna_region_from_url(source_url: str) -> dict[str, str]:
    ...
    # Current SEO route: /venta|arriendo/{tipo}/{region}/{comuna}/{hash}.
    m3 = re.search(r'/(?:venta|arriendo)/[^/]+/([^/]+)/([^/]+)/[^/]+$', low)
    if m3:
        result.setdefault("region", m3.group(1).replace("-", " ").title())
        result.setdefault("comuna", m3.group(2).replace("-", " ").title())
