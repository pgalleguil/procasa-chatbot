...
def query_summary(
    ...
    if user_filter:
        received_match_parts.append(user_filter)
    if extra:
        received_match_parts.append(extra)
    received_match = {"$and": received_match_parts} if len(received_match_parts) > 1 else received_match_parts[0]
...
def query_field_coverage(
    ...
    if user_filter:
        match_parts.append(user_filter)
    extra_filter = _build_extra_filter(filters)
    if extra_filter:
        match_parts.append(extra_filter)
                            ...
                            "$cond": [
                                {
                                    "$and": [
                                        {"$ne": [{"$ifNull": [expr, ""]}, ""]},
                                        *([{"$not": [{"$in": [expr, UNASSIGNED_VALUES]}]}] if name == "ejecutivo_asignado" else []),
                                    ]
                                },
                                1,
                                0,
                ...
                "conflict": {
                    "$sum": {
                        "$cond": [
                            {
                                "$and": [
                                    {"$ne": [{"$ifNull": ["$pipeline_stage", ""]}, ""]},
                                    {"$ne": [{"$ifNull": ["$stage", ""]}, ""]},
                                    {"$ne": ["$pipeline_stage", "$stage"]},
                                ]
                            },
                            1,
                            0,
...
def _build_extra_filter(filters: Optional[dict]) -> dict:
    """Filtros adicionales permitidos por el dashboard (todos read-only)."""
    conditions = {}
    if not filters:
        return conditions
...
def query_funnel(
    ...
    assigned = cohort_count({"ejecutivo_asignado": {"$nin": UNASSIGNED_VALUES, "$exists": True}})
    advanced = cohort_count({"$expr": {"$in": [eff, ADVANCED_STAGES]}})
    won = cohort_count({"$expr": {"$eq": [eff, "CLOSED_WON"]}})

    values = [("received", "Recibidos", received), ("assigned", "Asignados actualmente", assigned),
              ("advanced", "Avanzados actualmente", advanced), ("won", "Cerrados ganados actualmente", won)]
    funnel = []
    for index, (stage, label, count) in enumerate(values):
        previous = values[index - 1][2] if index else count
        funnel.append({"stage": stage, "label": label, "count": count,
                       "pct_of_received": round(count / received * 100, 1) if received else 0,
                       "difference_from_previous": count - previous})
...
def query_property_ranking(
    ...
    pipeline = [
        ...
        *([{"$match": extra_filter}] if extra_filter else []),
        {"$addFields": {
            ...
            "_is_advanced": {"$cond": [{"$in": [_effective_stage_expr(), ADVANCED_STAGES]}, 1, 0]},
        }},
        {"$group": {
            "_id": "$_code",
            "count": {"$sum": 1},
            ...
            "sources": {"$push": {"$ifNull": ["$prospecto.origen", "Sin informacion"]}},
        }},
        {"$sort": {"count": -1}},
        {"$limit": 11},
    ]
    ...
    for r in raw:
        ...
        dominant = max(set(srcs), key=srcs.count) if srcs else "Sin informacion"
        ranking.append({
            "code": code,
            "count": total,
...
def query_executive_load_detail(
    ...
    user_filter = _build_user_filter(executive)
    extra_filter = _build_extra_filter(filters)
    parts = [active] + ([user_filter] if user_filter else []) + ([extra_filter] if extra_filter else [])
    match = {"$and": parts} if len(parts) > 1 else active
    now_u = datetime.now(timezone.utc)
    ...
    pipeline = [
        ...
        {"$addFields": {
            ...
            "_is_hot": {"$cond": [{"$eq": ["$lead_temperature_effective", "HOT"]}, 1, 0]},
            "_is_pending": {"$cond": [{"$in": [_effective_stage_expr(), ["NEW", "SIN_ETAPA", None, ""]]}, 1, 0]},
        ...
        {"$group": {
            ...
            "active": {"$sum": 1},
            "hot": {"$sum": "$_is_hot"},
            "initial_stage": {"$sum": "$_is_pending"},
            "stalled": {"$sum": {"$cond": [{"$and": [{"$eq": ["$_is_pending", 1]}, {"$gt": ["$_age_days", 7]}]}, 1, 0]}},
...
def query_entry_pulse(
    executive: Optional[str] = None,
    filters: Optional[dict] = None,
) -> dict:
    """Pulso de entrada: hoy vs ayer misma hora, semana vs semana anterior, mes vs mes anterior."""
    db = get_db()
    user_filter = _build_user_filter(executive)
    extra_filter = _build_extra_filter(filters)
    ...
    def _count_in_range(start_dt, end_dt):
        if end_dt <= start_dt:
            return 0
    ...
    today = _count_in_range(today_start, now_cl)
    yesterday = _count_in_range(yesterday_start, yesterday_end)
    week = _count_in_range(monday_start, now_cl)
    prev_week = _count_in_range(prev_monday_start, prev_week_cut)
    month = _count_in_range(month_start, now_cl)
    prev_month = _count_in_range(prev_month_start, prev_month_cut)

    return {
        "today": today,
        "yesterday_same_cut": yesterday,
        "daily_variation_pct": round((today - yesterday) / yesterday * 100, 1) if yesterday else None,
        "current_week": week,
        "previous_week_same_cut": prev_week,
        "weekly_variation_pct": round((week - prev_week) / prev_week * 100, 1) if prev_week else None,
        "current_month": month,
        "previous_month_same_cut": prev_month,
        "monthly_variation_pct": round((month - prev_month) / prev_month * 100, 1) if prev_month else None,
    }


def query_entry_forecast(
    executive: Optional[str] = None,
    filters: Optional[dict] = None,
) -> dict | None:
    """Proyeccion simple de entrada 7 dias (promedio movil + ajuste dia de semana)."""
    db = get_db()
    user_filter = _build_user_filter(executive)
    extra_filter = _build_extra_filter(filters)
    now_cl = datetime.now(CHILE_TZ)
    start_60d = (now_cl - timedelta(days=60)).replace(hour=0, minute=0, second=0, microsecond=0)
    start_60d = CHILE_TZ.localize(start_60d)
    ...
    recent_total = sum(daily.get(d, 0) for d in dates[-7:])
    avg = recent_total / 7.0
    ...
    for date_str, count in daily.items():
        ...
        dow = dt.weekday()
        dow_counts.setdefault(dow, []).append(count)

    dow_factors = {}
    for dow, counts in dow_counts.items():
        dow_factors[dow] = (sum(counts) / len(counts)) / overall_avg if overall_avg else 1

    projected = []
    for i in range(1, 8):
        fd = now_cl + timedelta(days=i)
        factor = dow_factors.get(fd.weekday(), 1)
        projected.append(round(avg * factor))

    total_est = sum(projected)
    band = round(total_est * 0.25)
    return {
        "projected_daily": projected,
        "total_range": {"min": max(0, total_est - band), "max": total_est + band},
        "method": "7-day moving average + day-of-week adjustment",
...
def query_priorities(
    ...
    alerts = [
        ...
        },
        {
            ...
            "description": "Leads Hot cuya etapa efectiva actual es NEW",
            "count": _count({
                "lead_temperature_effective": "HOT",
                "$expr": {"$eq": [_effective_stage_expr(), "NEW"]},
            }),
        },
        {
        ...
        },
        {
            ...
            "count": _count({
                "ejecutivo_asignado": {"$in": ["Sin Asignar", "No Asignado", None, ""]},
                "$expr": {"$lt": ["$_created_normalized", cutoff_48h]},
            }),
        },
        {
            "type": "new_over_7d",
            ...
            "count": _count({
                "$expr": {"$and": [{"$in": [_effective_stage_expr(), ["NEW", "SIN_ETAPA", None, ""]]}, {"$lt": ["$_created_normalized", cutoff_7d]}]},
            }),
        },
        {
            "type": "no_property_code",
            "severity": "low",
