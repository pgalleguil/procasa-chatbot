...
def build_html(
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#ffffff;border:1px solid #d7e0eb;border-radius:14px;margin:0 0 22px 0;">
          <tr>
            <td style="padding:18px;">
              ...
              <p style="margin:0 0 10px 0;font-size:14px;line-height:1.6;color:#475569;">Propiedades similares en {comuna} muestran mejor desempeño comercial en rangos entre <b>{rango_min:,.0f} UF</b> y <b>{rango_max:,.0f} UF</b>.</p>
    ...
    if usar_ruta_tasacion:
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#ffffff;border:1px solid #e2e8f0;border-radius:16px;margin:0 0 26px 0;box-shadow:0 4px 10px rgba(0,0,0,0.03);">
          <tr>
            <td style="padding:22px;">
              <!-- Cabecera -->
              <p style="margin:0 0 4px 0;font-size:18px;line-height:1.35;color:#0f172a;font-weight:700;letter-spacing:-0.01em;">Evaluación comercial de precio</p>
              ...
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 16px 0;">
                <tr>
                  <td style="width:48%;vertical-align:top;background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:12px 14px;">
                    ...
                    <p style="margin:0;font-size:22px;color:#475569;font-weight:700;">{precio:,.0f} <span style="font-size:16px;font-weight:600;">UF</span></p>
                  </td>
                  <td style="width:4%;"></td>
                  <td style="width:48%;vertical-align:top;background:#f0f7ff;border:1px solid #bfdbfe;border-radius:12px;padding:12px 14px;">
                    ...
                    <p style="margin:0;font-size:22px;color:#2563eb;font-weight:700;">{precio_sugerido_tasacion:,.0f} <span style="font-size:16px;font-weight:600;">UF</span></p>
                  </td>
                </tr>
              </table>
              ...
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 20px 0;border-top:1px solid #f1f5f9;">
                <tr>
                  ...
                  <td align="right" style="padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:13px;font-weight:700;color:#0f172a;">{f"{tasacion_min:,.0f} - {tasacion_max:,.0f} UF" if tasacion_min is not None and tasacion_max is not None else "No disponible"}</td>
                </tr>
                <tr>
                  ...
                  <td align="right" style="padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:13px;font-weight:700;color:#0f172a;">
                    <span style="color:#ef4444;font-size:11px;margin-right:4px;vertical-align:middle;">▲</span>{brecha_tasacion_pct:+.1f}%
                  </td>
                </tr>
                <tr>
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#ffffff;border:1px solid #e2e8f0;border-radius:16px;margin:0 0 26px 0;box-shadow:0 4px 10px rgba(0,0,0,0.03);">
          <tr>
            <td style="padding:22px;">
              <!-- Cabecera -->
              <p style="margin:0 0 4px 0;font-size:18px;line-height:1.35;color:#0f172a;font-weight:700;letter-spacing:-0.01em;">{asunto_tag}</p>
              <p style="margin:0 0 20px 0;font-size:13px;line-height:1.5;color:#64748b;">
              ...
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 16px 0;">
                <tr>
                  ...
                  <td style="width:48%;vertical-align:top;background:#f0f7ff;border:1px solid #bfdbfe;border-radius:12px;padding:12px 14px;">
                    ...
                    <p style="margin:0;font-size:22px;color:#2563eb;font-weight:700;">{nuevo:,.0f} <span style="font-size:16px;font-weight:600;">UF</span></p>
                  </td>
                </tr>
              </table>
        ...
        brecha_ic_txt = f"{brecha_ic_val:+.1f}%" if (brecha_ic_val is not None and abs(brecha_ic_val) >= 0.1) else "No disponible"
        ufm2_pub_txt = f"{float(uf_m2_publicacion_actual):,.2f} UF/m2" if uf_m2_publicacion_actual is not None else "No disponible"
        ufm2_cierre_txt = f"{float(uf_m2_venta_efectiva_actual):,.2f} UF/m2" if uf_m2_venta_efectiva_actual is not None else "No disponible"
        activas_txt = f"{int(publicaciones_activas):,}" if publicaciones_activas is not None else "No disponible"
        variacion_ufm2_12m = prop.get("variacion_uf_m2_12m")
        variacion_ufm2_12m_txt = (
            f"{float(variacion_ufm2_12m):+.1f}%"
        ...
        lectura_brecha = (
            f"En propiedades similares de {comuna}, los cierres efectivos recientes se observan en promedio {abs(brecha_ic_val):.1f}% "
            f"{'bajo' if brecha_ic_val < 0 else 'sobre'} los valores iniciales de publicacion."
            if (brecha_ic_val is not None and abs(brecha_ic_val) >= 0.1)
            else "La brecha entre publicacion y cierre efectivo se mantiene en observacion con los datos actualmente disponibles."
        )
        lectura_competencia = (
            f"Actualmente se observa {('aumento' if str(prop.get('tendencia_publicaciones') or '').strip().lower() == 'alza' else 'estabilidad')} "
        ...
        contexto_comercial_fallback = (
            f"El mercado de {tipo.lower()} en {comuna} muestra un entorno competitivo que requiere foco en diferenciacion comercial y calidad de publicacion. "
            "La recomendacion se orienta a fortalecer posicionamiento, propuesta de valor y gestion activa de oportunidades."
        )
        recomendaciones_html = f"""
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#ffffff;border:1px solid #e2e8f0;border-radius:16px;margin:0 0 26px 0;box-shadow:0 4px 10px rgba(0,0,0,0.03);">
          <tr>
            <td style="padding:22px;">
              <!-- Cabecera -->
              <p style="margin:0 0 4px 0;font-size:18px;line-height:1.35;color:#0f172a;font-weight:700;letter-spacing:-0.01em;">Evaluación de inteligencia comercial de mercado</p>
              <p style="margin:0 0 20px 0;font-size:13px;line-height:1.5;color:#64748b;">
              ...
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 20px 0;border-top:1px solid #f1f5f9;">
                <tr>
                  ...
                  <td align="right" style="padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:13px;font-weight:700;color:#0f172a;">{liquidez or "No disponible"}</td>
                </tr>
                <tr>
                  <td style="padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:13px;color:#475569;">Nivel de competencia</td>
                  <td align="right" style="padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:13px;font-weight:700;color:#0f172a;">{nivel_competencia or "No disponible"}</td>
                </tr>
                <tr>
                  <td style="padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:13px;color:#475569;">Presión comercial en zona</td>
                  <td align="right" style="padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:13px;font-weight:700;color:#0f172a;">{score_presion_txt}</td>
                </tr>
                <tr>
                ...
                <tr>
                  ...
                  <td align="right" style="padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:13px;font-weight:700;color:#0f172a;">{activas_txt}</td>
                </tr>
                <tr>
    ...
    if incluye_tasacion_adjunta:
        texto_tasacion_adjunta = (
            "Adjuntamos una tasacion comercial desarrollada por Propiteq, plataforma especializada en analisis de mercado inmobiliario. "
              ...
              <table role="presentation" width="640" cellspacing="0" cellpadding="0" style="max-width:640px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e2e8f0;">
                ...
                <tr>
                  <td style="padding:34px 22px 28px 22px;">
                    <h1 style="margin:0 0 12px 0;font-size:28px;line-height:1.25;color:#0f172a;font-weight:700;">{"Detectamos una oportunidad de alineamiento comercial respecto al mercado actual" if usar_ruta_tasacion else "Detectamos una oportunidad para mejorar el desempeño comercial de tu propiedad"}</h1>
                    ...
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;margin:0 0 22px 0;">
                      <tr>
                        <td style="padding:18px;">
                          <p style="margin:0 0 14px 0;font-size:16px;line-height:1.3;color:#0f172a;font-weight:700;letter-spacing:-0.01em;">Propiedad en seguimiento</p>
                          <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                            <tr>
                              <td style="width:33%;vertical-align:top;padding-right:10px;">
                                ...
                                <p style="margin:0;font-size:15px;font-weight:700;color:#0f172a;"><a href="{property_url}" style="color:#0f172a;text-decoration:none;border-bottom:1px dashed #cbd5e1;">{codigo}</a></p>
                              </td>
                              <td style="width:33%;vertical-align:top;padding-right:10px;">
                                <p style="margin:0 0 4px 0;font-size:12px;color:#64748b;text-transform:uppercase;letter-spacing:0.04em;">Tipo</p>
                                <p style="margin:0;font-size:15px;font-weight:700;color:#0f172a;">{tipo}</p>
                              </td>
                              <td style="width:34%;vertical-align:top;">
                                <p style="margin:0 0 4px 0;font-size:12px;color:#64748b;text-transform:uppercase;letter-spacing:0.04em;">Comuna</p>
                                <p style="margin:0;font-size:15px;font-weight:700;color:#0f172a;">{comuna}</p>
                              </td>
                            </tr>
                          </table>
                        </td>
                    ...
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 12px 0;">
                      <tr>
                        <td>{btn(urls['aceptar_rebaja'], 'Revisar posicionamiento sugerido', '#0f172a', css_class='btn-primary')}</td>
                      </tr>
                    </table>

                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 28px 0;">
                      <tr>
                        <td>{btn(urls['contactar_ejecutivo'], 'Evaluar recomendacion con asesor', '#ffffff', '1px solid #94a3b8', '#0f172a', css_class='btn-secondary')}</td>
                      </tr>
                    </table>
