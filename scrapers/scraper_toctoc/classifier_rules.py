...
STRONG_PUBLISHER_FIELDS = (
    ...
    "seller_profile_url", "seller_profile_logo", "seller_type_evidence",
    "operation_label_raw",
)
WEAK_CONTEXT_FIELDS = (
...
def classify_obvious_broker(extracted: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return known_broker_brand_in_strong_fields(extracted) or hard_broker_terms_in_strong_fields(extracted)


def build_deepseek_context(extracted: dict[str, Any]) -> dict[str, Any]:
    context = _all_evidence_context(extracted)
    return {k: extracted.get(k, "") for k in ("publicador_visible", "contact_name", "contact_logo_alt", "seller_type", "listing_advertiser", "seller_jsonld_name", "contact_badges_text", "seller_profile_url", "seller_profile_logo", "seller_type_evidence", "operation_label_raw")} | {
