#!/usr/bin/env python3
# email_campaña_propietarios.py → VERSIÓN TERCER RECORDATORIO (ENERO 2026 - RM)
...
NOMBRE_ANTERIOR_1 = "ajuste_precio_202512"              # Primer envío
NOMBRE_ANTERIOR_2 = "ajuste_precio_202512_REPASO"       # Segundo envío (si existe)
MODO_PRUEBA = True                    
...
# ==============================================================================
# GENERACIÓN DE HTML (DATOS ACTUALIZADOS ENERO 2026 + RECORDATORIO)
# ==============================================================================
def generar_html(nombre, propiedades, email_real):
    ...
    codigos_html = ", ".join(codigos_con_link)
    ...
    <head>
        ...
        <style>
            ...
            strong {{ color:#0F172A; }}
            .alert-box {{ background:#FEF2F2; border:1px solid #FECACA; padding:20px; margin:28px 0; border-radius:8px; color:#991B1B; font-weight:600; }}
        ...
        <div class="wrapper">
            <div class="card">
                ...
                <div class="content">
                    ...
                    <div class="alert-box">
                        ⚠️ Recordatorio final: No hemos recibido respuesta a los correos anteriores sobre la revisión de precio. Esta es la última comunicación antes de archivar esta oportunidad estratégica.
                    </div>
                    
                    <p>Le recordamos que hemos analizado {texto_prop} (<strong>{codigos_html}</strong>). A pesar de su calidad, sigue sin el movimiento esperado. El mercado inmobiliario en la Región Metropolitana continúa competitivo, y un ajuste estratégico puede acelerar significativamente la venta en este inicio de 2026.</p>
                    
                    <div class="highlight-box">
                        <strong>Mercado RM enero 2026:</strong> Stock elevado (>70.000 unidades nuevas acumuladas, datos recientes).<br><br>
                        <strong>Tiempos de venta:</strong> Promedio ~140 días para departamentos (tendencia a la baja, pero aún extendidos).<br><br>
                        <strong>Tasas hipotecarias:</strong> Estables en ~4,1% (Banco Central, cierre 2025 / inicio 2026), con proyecciones positivas para el año.<br><br>
                        <strong>Señales de recuperación:</strong> Mayor demanda proyectada para 2026 (CChC / Colliers), pero la competitividad sigue siendo clave para capturar compradores activos.
                    </div>
                    
                    <p>Recomendamos un <strong>ajuste del 7%</strong> para posicionar mejor la propiedad en este momento de reactivación. Experiencia en transacciones similares muestra mayor visibilidad y cierre más rápido.</p>
                    ...
                    <p style="text-align:center; font-size:17px; font-weight:700; color:#0F172A; margin:30px 0 20px;">
                        Por favor, seleccione una opción (respuesta inmediata):
                    </p>
                    ...
                    <p style="text-align:center; font-size:14.5px; color:#475569; margin:16px 0 0;">
                        Un clic es suficiente • Datos basados en fuentes oficiales (Banco Central, CChC, Colliers – enero 2026)
                    </p>
                </div>
                <div class="footer">
                    ...
                    <div style="margin-top:20px; font-size:11px; color:#64748B; line-height:1.6;">
                        © 2026 Procasa – Todos los derechos reservados<br>
...
# ==============================================================================
# MAIN (TERCER ENVÍO RM - ENERO 2026)
# ==============================================================================
def main():
    print("="*70)
    print("CAMPAÑA TERCER RECORDATORIO PRECIO - RM")
    ...
    pipeline = [
        {
            "$match": {
                ...
                "update_price.campana_nombre": {"$in": [NOMBRE_ANTERIOR_1, NOMBRE_ANTERIOR_2]},
                "estado": {"$exists": False}  # ← EXCLUYE a quienes YA respondieron cualquier opción
            }
        },
        {
            "$lookup": {
        ...
        },
        {
            "$match": {
                "$or": [
                    ...
                    {"propiedad_info.region": "Región Metropolitana de Santiago"}
                ]
            }
        },
        {
    ...
    print(f"Correos únicos a enviar (solo sin respuesta): {len(grouped_data)}")
    ...
    for email_real, data in grouped_data.items():
        ...
        if len(data['propiedades']) > 1:
            asunto = f"ÚLTIMO RECORDATORIO: Revisión precio propiedades (ej. {codigo_principal}) – 2026"
        else:
            asunto = f"ÚLTIMO RECORDATORIO: Revisión precio {codigo_principal} – Oportunidad 2026"
        ...
        if debe_actualizar:
            collection.update_many(
                ...
                {"$set": {
                    ...
                    "update_price.canales.email_tercer": {"enviado": exito, "fecha": ahora}
                }}
            )
